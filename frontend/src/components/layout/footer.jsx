...
export function Footer({ compact = false }) {
  ...
  return (
    <footer className="border-t border-border bg-background">
      ...
      {/*
        One trust card, not a bare row: what a shop takes and how it is protected read as
        a single claim, and the boxed card is what separates it from the sitemap above.

        Payment marks run FULL WIDTH here rather than inside the brand column. [MEASURED]
        in a five-column grid that column is about 190px, and "American Express" is wider
        than the column on its own — so `flex-wrap` gave every badge a row of its own and
        the result read as five stacked boxes.
      */}
